...
class SurfaceEnergy(Property):
    def __init__(self, parameter, inter_param=None):
        ...
        if not self.reprod:
            if not ("init_from_suffix" in parameter and "output_suffix" in parameter):
                ...
                parameter["initial_vacuum_size"] = parameter.get("initial_vacuum_size", 0)
                self.initial_vacuum_size = parameter["initial_vacuum_size"]
        ...
        if self.reprod:
            ...
            if refine:
                ...
                for ii in task_list_basename:
                    ...
                    if os.path.isfile("surface_energy.json"):
                        os.remove("surface_energy.json")
                    if os.path.islink("surface_energy.json"):
                        os.remove("surface_energy.json")
                    os.symlink(
                        os.path.relpath(os.path.join(init_from_task, "surface_energy.json")),
                        "surface_energy.json",)
            else:
                ...
                vacuum = []
                num = 1
                while self.vacuum_size_step * num + self.initial_vacuum_size <= self.max_vacuum_size:
                    vacuum_size = self.vacuum_size_step * num + self.initial_vacuum_size
                    gen = SlabGenerator(
                        ss,
                        self.miller_index,
                        self.min_slab_size,
                ...
                for ii in range(len(all_slabs)):
                    ...
                    if self.inter_param["type"] == "abacus":
                        ...
                        os.remove("POSCAR")
                    surface_energy = {"miller_index": all_slabs[ii].miller_index, "vacuum_size":vacuum[ii]}
                    dumpfn(surface_energy, "surface_energy.json", indent=4)
        os.chdir(cwd)
        return task_list
    ...
    def _compute_lower(self, output_file, all_tasks, all_res) -> [dict, str]:
        ...
        if not self.reprod:
            ...
            ptr_data += "Vacuum_size(e-10 m):\tSurf_E(J/m^2) EpA(eV) equi_EpA(eV)\n"
            ...
            for ii in all_tasks:
                ...
                vacuum_size = loadfn(os.path.join(ii, "surface_energy.json"))["vacuum_size"]
                ptr_data += "%-30s     %7.3f     %8.3f     %8.3f\n" % (
